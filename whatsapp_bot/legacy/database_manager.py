...
class DatabaseManager:
    ...
    # NOTE: Preference extraction is now handled entirely by AI through PREFERENCES_DETECTED
    # The methods below are kept for potential fallback scenarios but are not actively used
    ...
    def extract_preferences_from_text(self, text: str) -> Dict:
        ...
        # Standardized job types with comprehensive variations
        job_types = {
            "frontend developer": [
                "frontend",
                "front-end",
        ...
        # Find the best matching job type using fuzzy matching
        best_match = self.find_best_job_type_match(text_lower, job_types)
        if best_match:
            preferences["job_type"] = best_match
