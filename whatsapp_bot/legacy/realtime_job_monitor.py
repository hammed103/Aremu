...
class RealTimeJobMonitor:
    ...
    def __init__(self, whatsapp_bot=None):
        ...
        # Monitoring settings
        self.min_match_score = 39  # Minimum match score to send
    ...
    def check_job_matches_for_user(self, user: Dict, new_jobs: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"❌ Error checking matches for user {user.get('user_id')}: {e}")
            return []
    ...
    def send_real_time_job_alert(self, user: Dict, job: Dict) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"🚨 Sent real-time alert to {phone_number} for job {job['id']}")
        ...
        try:
            ...
            for user in active_users:
                try:
                    ...
                    # Check daily limit
                    if not self.job_tracker.should_send_more_jobs(user_id, self.max_jobs_per_user_per_day):
                        continue
    ...
    def start_continuous_monitoring(self):
        ...
        logger.info(f"🚀 Starting continuous job monitoring (every {self.check_interval}s)")
